...
public List<BookDTO> listBooks() throws SystemException {
        try {
            List<Book> books = bookRepository.findAll(); // Получаем сущности Book
            return books.stream()
                        .map(this::convertToDTO) // Преобразуем каждую книгу в BookDTO
                        .collect(Collectors.toList());
        } catch (Exception e) {
            throw new SystemException("Ошибка при получении книги", e);
        }
    }
